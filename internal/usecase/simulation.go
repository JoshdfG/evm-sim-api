...
// List returns paginated simulation history for an owner (API key holder).
func (uc *SimulationUseCase) List(ctx context.Context, ownerID string, limit, offset int) ([]entity.SimulationResult, error) {
	results, err := uc.repo.ListByAPIKey(ctx, ownerID, limit, offset)
	if err != nil {
		return nil, fmt.Errorf("simulation: List: %w", err)
	}
	if results == nil {
		results = []entity.SimulationResult{} // never return null JSON array
	}
	return results, nil
}
